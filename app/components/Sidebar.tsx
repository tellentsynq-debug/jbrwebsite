    ...
    >
      <div style={{ padding: "24px", display: "flex", alignItems: "center", justifyContent: isCollapsed ? "center" : "space-between", borderBottom: `1px solid ${C.border}`, height: "85px" }}>
        ...
        <motion.button
          ...
          whileHover={{ scale: 1.05, backgroundColor: C.inputBg }}
          whileTap={{ scale: 0.95 }}
          ...
          <div key={gIdx} style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {!isCollapsed ? (
              <motion.span
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                style={{ fontSize: "10px", fontWeight: 600, letterSpacing: "1px", textTransform: "uppercase", color: C.textHint, paddingLeft: "12px", marginBottom: "4px" }}
              >
                {group.group}
              </motion.span>
            ) : (
            ...
            {group.items.map((item) => {
              ...
              return (
                <motion.button
                  ...
                  whileHover={!isActive ? { backgroundColor: C.inputBg, x: 4 } : {}}
                  whileTap={{ scale: 0.98 }}
                  style={{
                ...
                >
                  {isActive && (
                    <motion.div
                      ...
                      style={{ position: "absolute", left: 0, top: "10%", bottom: "10%", width: "3px", borderRadius: "0 4px 4px 0", background: C.red }}
                    />
                  )}
                  <div style={{ display: "flex", justifyContent: "center", width: isCollapsed ? "100%" : "auto", color: isActive ? C.red : C.textMuted }}>
                  ...
                  <AnimatePresence>
                    {!isCollapsed && (
                      <motion.span
                        ...
                        animate={{ opacity: 1, x: 0 }}
                        exit={{ opacity: 0, x: -10 }}
                        style={{ fontSize: "13px", fontWeight: isActive ? 600 : 500, color: isActive ? C.red : C.textLabel, whiteSpace: "nowrap" }}
                      >
                        {item.label}
                      </motion.span>
                    )}
